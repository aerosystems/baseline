...
| Вимога | Опис |
|--------|------|
| **Знання** | Лекція «Криптосистеми RSA та Ель-Гамаля», ЛР2 (обернений елемент за модулем) |
...
- Якщо e·d ≡ 1 (mod φ(n)), то (Mᵉ)ᵈ ≡ M (mod n)
...
```
Зверніть увагу на четвертий пункт: дві літери l дали однакові шифроблоки 68313330. Посимвольне RSA-шифрування детерміноване й перетворюється на шифр простої заміни з 256 символів — частотний аналіз з ЛР1 зламає його так само, як шифр Цезаря. Тому в реальних системах RSA шифрує не текст, а один випадковий сеансовий ключ, і завжди з доповненням OAEP.
...
|----|----|----|
| 1 | p=61, q=53 | Перевірити, що (Mᵉ)ᵈ = M для M = 0, 1, n − 1, а також для M, кратних p. |
...
| 5 | p=97, q=101 | Порівняти час шифрування для e = 17, 257 і найбільшого допустимого e < φ(n). Чому e = 3 тут недопустиме і що важливіше для швидкості — величина e чи кількість одиниць у його двійковому записі? |
| 6 | p=101, q=107 | Реалізувати RSA з e = 3 і показати атаку: якщо M³ < n, то M відновлюється звичайним кубічним коренем з C. |
| 7 | p=109, q=113 | Показати, що знаючи φ(n), можна знайти p і q. |
| 8 | p=127, q=131 | Зашифрувати своє ім'я (як число). |
| 9 | p=137, q=139 | Додати до «текстбук RSA» випадкове доповнення (кілька випадкових бітів перед M) і показати, що однакові M тепер дають різні C. |
...
| 16 | p=211, q=223 | Перевірити, що D(E(M)) = M для всіх M < n, зокрема для M, не взаємно простих з n. |
...
│  │ Ключ: 2048 біт (мінімум до 2030), 3072 біти (рекомендовано)     ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
