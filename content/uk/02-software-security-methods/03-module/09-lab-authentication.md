...
equipment:
  ...
  - "Утиліта oathtool або застосунок-автентифікатор із підтримкою SHA256"
...
**Обладнання:** ПК з встановленим C++ компілятором (MSVC, MinGW або GCC); Середовище розробки (Visual Studio, VS Code); утиліта oathtool або застосунок-автентифікатор із підтримкою SHA256.
...
| Вимога | Опис |
|--------|------|
| **Знання** | Лекція «Ідентифікація та аутентифікація. Загрози безпеки», ЛР7 (SHA-256) |
...
Сіль захищає від райдужних таблиць, але не від перебору: одна відеокарта рахує мільярди SHA-256 за секунду. Тому хеш обчислюють багато разів (PBKDF2) або беруть функції, навмисно вимогливі до пам'яті (Argon2id). У прикладі нижче — спрощений аналог PBKDF2 зі 100 000 ітерацій.
...
2. HMAC: H = HMAC(secret_key, T) — за замовчуванням HMAC-SHA1,
   RFC 6238 дозволяє також SHA-256 і SHA-512
...
```
TOTP = HOTP(K, T) = Truncate(HMAC(K, T)) mod 10^6
...
Програма в завданні 4 використовує HMAC-SHA256, бо SHA-256 уже реалізовано в ЛР7. Застосунки, що підтримують параметр `algorithm=SHA256` (Aegis, 2FAS, FreeOTP), покажуть ті самі коди, а Google Authenticator завжди рахує HMAC-SHA1, тож з ним коди не збіжуться. Для перевірки реалізації RFC 6238 містить тестові вектори: з ключем `12345678901234567890123456789012` (ASCII, 32 байти), 8 цифрами й SHA-256 у момент T = 59 с код має дорівнювати **46119246**.
...
int main() {
    ...
    // Контроль за тестовим вектором RFC 6238 (SHA-256, 8 цифр, T = 59 с)
    std::string rfcKey = "12345678901234567890123456789012";
    TOTP rfc(Bytes(rfcKey.begin(), rfcKey.end()), 8);
    std::cout << "RFC 6238, T=59:   " << rfc.generate(59)
              << " (має бути 46119246)\n\n";
...
RFC 6238, T=59:   46119246 (має бути 46119246)
...
однаковими в усіх студентів будуть лише секрет, рядок контролю RFC 6238 і
структура виводу. Поточні коди можна звірити з еталонною утилітою, передавши
їй той самий секрет у шістнадцятковому вигляді:

```bash
oathtool --totp=sha256 53747564656e745365637265744b657931323334
```

Той самий секрет у Base32 для застосунку-автентифікатора —
`KN2HKZDFNZ2FGZLDOJSXIS3FPEYTEMZU`, алгоритм SHA256.

## Порядок виконання роботи

1. Отримати в викладача номер індивідуального варіанта.

2. Реєстрація користувача: реалізувати генерацію солі, хешування пароля з сіллю й ітераціями, показати, що однакові паролі з різною сіллю дають різні хеші.

3. Перевірка пароля: реалізувати перевірку з порівнянням хешів за сталий час і продемонструвати різницю з наївним порівнянням (завдання 2).

4. Challenge-Response: реалізувати протокол, показати успішну автентифікацію й відмову при повторному надсиланні старої відповіді.

5. TOTP: реалізувати генерацію кодів, перевірити реалізацію тестовим вектором RFC 6238 і утилітою `oathtool --totp=sha256`, продемонструвати повний цикл 2FA (пароль + TOTP).
...
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │ counter = time() // 30                                          ││
│  │ code = Truncate(HMAC(secret, counter)) mod 10^6 (SHA-1/256/512) ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                     │
│  ФАКТОРИ MFA:                                                       │
│  ┌─────────────────────────────────────────────────────────────────┐│
