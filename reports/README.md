...
```
reports/<курс>/labs/<група>/<номер роботи>/<логін GitHub>/
    report.md          ← ваш звіт
    assets/           ← знімки екрана
    src/               ← вихідники програми
    ЛР03_login.docx    ← збирає конвеєр, комітити не треба
```
...
`reports/<курс>/labs/<група>/students.json`. Номер студента в цьому файлі визначає
шифр роботи на титулці (`ФКЗЕ. 121ПМЗІ<номер>`), а поле `github` студент вписує
сам — доки воно порожнє, тека зветься `student-NN`.
